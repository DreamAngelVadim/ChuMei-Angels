...
class ChuMei:
    ...
    async def _speak(self, text, voice=None, duet=False):
        ...
        # Заменяем ЛЮБЫЕ числа от 10 до 9999
        text = re.sub(r'\b\d{2,4}\b', replace_number, text)
        
        # Заменяем числа, прилипшие к словам (200долларов → двестидолларов)
        text = re.sub(r'(\d{2,4})([а-яА-Я])', lambda m: replace_number(match=m) + m.group(2), text)
