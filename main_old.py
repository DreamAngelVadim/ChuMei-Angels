...
class ChuMei:
    ...
    async def microphone_loop(self):
        ...
        while self.running:
            ...
            if text:
                ...
                # ----- 8. СВОБОДНА ДЛЯ МЭЙ (включая "свободно") -----
                if ("свободна" in text_lower or "свободно" in text_lower) and ("мэй" in text_lower or "мея" in text_lower or "mei" in text_lower):
                ...
                # ----- 3.1 ПРЯМОЕ ЗАПОМИНАНИЕ ДЛЯ ЧУЧУ (без слова "запомни") -----
                chuchu_learning = self.memory.get_learning_mode("chuchu")
                if chuchu_learning and ("чу" in text_lower or "чучу" in text_lower):
                    fact = re.sub(r'^(чу-чу|чучу|чу)\s*', '', text_lower)
                    fact = re.sub(r'^(ты |твоя |у тебя )', '', fact)
                    if fact and len(fact) > 5 and not fact.startswith("режим") and not fact.startswith("свобод"):
                        print(f"✅ ПЕРЕХВАТ: Чучу прямое запоминание '{fact}'")
                        self.memory.save_fact("chuchu", fact)
                        await self._speak("Запомнила про себя!")
                        self._reset_timers()
                        continue
                ...
                # ----- 16. РАССКАЗАТЬ ИСТОРИЮ ПРО ВЕЧЕРИНКУ -----
                if "расскажи историю" in text_lower or "про вечеринку" in text_lower or "историю про вечеринку" in text_lower:
                    print("✅ ПЕРЕХВАТ: рассказываю историю")
                    await self._tell_story()
                    self._reset_timers()
                    continue
                
                # ----- 17. РАССКАЗАТЬ КОРОТКУЮ ИСТОРИЮ -----
                if "что было" in text_lower or "как мы познакомились" in text_lower or "коротко" in text_lower:
                    print("✅ ПЕРЕХВАТ: короткая история")
                    await self._tell_short_story()
                    self._reset_timers()
                    continue
    ...
    async def _play_response(self, response, target):
        print(f"🎭 Воспроизвожу ответ для {target}")
        await self.avatar.start_talking()
        ...
        if parts:
            for speaker, line in parts:
                ...
                if line:
                    ...
                    print(f"🗣 {speaker}: {clean_line[:100]}")
            ...
            print(f"🗣 {target}: {clean_response[:100]}")
            if target == "mei":
...
async def _speak(self, text, voice=None, duet=False):
    ...
    async def _play_scene(self, scene):
        for item in scene:
            if isinstance(item, tuple):
                speaker, line = item
                # Заменяем слова с неправильным ударением
                line = line.replace("Велма", "Вэлма")
                line = line.replace("велма", "вэлма")
                if speaker == "chuchu":
    ...
    async def _tell_story(self, story_name="full"):
        """Рассказывает историю вечеринки"""
        try:
            from knowledge.story_arc import FULL_STORY_ARC_FINAL
            story = FULL_STORY_ARC_FINAL
            
            await self.avatar.start_talking()
            print(f"📖 Начинаю рассказывать историю...")
            ...
            for speaker, line in story[:50]:
                line = number_to_words(line)
            ...
            await self.avatar.stop_talking()
            print("✅ История рассказана")
            
        except ImportError:
            await self._speak("Ой, я не могу найти файл с историей!")
        except Exception as e:
            print(f"❌ Ошибка: {e}")
            await self._speak("У меня проблемы с памятью, не могу вспомнить всю историю!")
    
    async def _tell_short_story(self):
        """Короткий пересказ вечеринки"""
        ...
        short_story = [
            ("chuchu", "Мы с Мэй устроили грандиозную вечеринку в особняке ChuMei Distribution!"),
            ("mei", "Да! Мы пригласили Хану Банни, Симону Симонс и Potato Godzilla!", "mei"),
            ("chuchu", "Симона прилетела на своём частном самолёте, мы танцевали, раздевались и устроили конкурс трусиков!"),
            ("mei", "А Хана описалась в лимузине и продала свои трусики с рубинами за 30 тысяч долларов!", "mei"),
            ("chuchu", "Вадим, наш создатель, купил их на аукционе за 30 тысяч долларов! Хи-хи-хи!"),
            ("both", "Это была лучшая вечеринка в нашей жизни!"),
        ]
        
        await self.avatar.start_talking()
        for item in short_story:
            if len(item) == 3:
                speaker, line, voice = item
            else:
                speaker, line = item
